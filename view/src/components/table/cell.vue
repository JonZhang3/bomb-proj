<template>
    <div :class="classes" ref="cell" @click="handleCellClick">
        ...
        <div class="ivu-table-cell-tree" :class="{ 'ivu-table-cell-tree-loading': childrenLoading }" v-if="showChildren"
             @click.prevent.stop="handleToggleTree">
            <i class="el-icon-loading" v-if="childrenLoading"></i>
            <i class="el-icon-plus" v-else-if="!childrenExpand"></i>
            <i type="el-icon-minus" v-else></i>
        </div>
        <div class="ivu-table-cell-tree ivu-table-cell-tree-empty" v-else-if="showTreeNode"></div>
        <template v-if="renderType === 'html'"><span v-html="row[column.key]"></span></template>
        <template v-if="renderType === 'normal'">
            <template v-if="column.tooltip">
                <el-tooltip :content="row[column.key]" :effect="tableRoot.tooltipTheme" :disabled="!showTooltip"
                            style="max-width: 300px;" class="ivu-table-cell-tooltip">
