<template>
    <transition name="drawer" @before-enter="drawerBeforeEnter" @enter="drawerEnter" @after-enter="drawerAfterEnter"
                @leave="drawerleave" @before-leave="drawerbeforeLeave" v-on:after-leave="drawerafterLeave">
        <div class="bomb-drawer" v-if="visible" @click="drawerHide"
             :style="computeStyle()">
            <transition name="drawerC" @before-enter="beforeEnter" @enter="enter" @after-enter="afterEnter"
                        @leave="leave" @after-leave="afterLeave">
                <div
                    :class="['Eject',aligns === 'left'?'EjectLeft':aligns === 'right'?'EjectRight':aligns === 'top'?'EjectTop':aligns === 'bottom'?'EjectBottom':'EjectRight']"
                    :style="{'width':widths,'height':heights}" v-if="jdShow" @click.stop="eject" ref="eject">
                    <!--加载中-->
                    <div class="bomb-draw-loading" v-if="loading === undefined ? false : loading">
                        <div class="bomb-draw-loader" title="2">
                            <svg version="1.1" id="loader-1" x="0px" y="0px" width="40px" height="40px"
                    ...
                    <header class="bomb-draw-header"
                        v-if="headerShow === undefined?true:headerShow"
                        :style="{'background':headerBackground?headerBackground:'#fff'}">
                        <div class="bomb-draw-headDiv" v-if="!headerSlot">
                            <span class="bomb-draw-title" :style="{'color':titleColor?titleColor+'!important':''}">{{title?title:"标题"}}</span>
                            <span @click.stop="close" class="bomb-draw-close"
                                  v-if="closeBtnShow === undefined?true:closeBtnShow">x</span>
                        </div>

                        <slot name="header"></slot>
                    </header>
                    <main class="bomb-draw-main" :style="{'background':mainBackground?mainBackground:'#fff'}">
                        <slot></slot>
                    </main>
                    <footer class="bomb-draw-footer"
...
<script>
    export default {
        ...
        props: [
            ...
            "footerShow",
            "footerHeight",
            "width", "height", "visible", "footerBackground", "headerShow", "headerBackground", "titleColor",
            "mainBackground", "align", "closeOnClickModal", "loading", "loadingColor", "modal"
        ],
        data() {
            return {
                jdShow: false,
